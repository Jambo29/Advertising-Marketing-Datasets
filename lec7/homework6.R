...
# Build models on sample
	...
	colnames(coefs) <- c("OLS", "LASSO", "ELASTIC_NET"); coefs
...
# Create lift charts
	...
	# Predictive Modeling
	trainlogit <- train[ ,spend := ifelse(outcome_spend > 0, 1, 0)] # make spend indicator NULL, unknown at time
	trainlogit <- trainlogit[ ,outcome_spend := NULL][]
...
# Lift tables
$$\alpha \beta ^{2} + \left |\beta \right |$$.

![Penalty Weights](home/nmatare/projects/datascience_marketing/lec7/penalty.png)